let app = (() => {
  ...
  App.namespace('defaultPartial.mineQuestion');
	...
	// App.urls['origin'] = 'http://192.168.2.147:8080/';
  ...
  App.namespace('defaultPartial.template');  // 测试
	...
	let globalCache = {
	  userinfo: {  // 用户信息
		...
		hotQuestionDataCount: 0,  // 第一次获取热门提问的初始值
		newestQuestionDataCount: 0,  // 第二次获取热门提问的初始值
		mineQuestionDataCount: 0  // 第三次获取热门提问的初始值
	};  // 全局缓存对象
  ...
  updating.init = () => {
		...
		*/
		...
		const getEmailCode = (() => {  // 获取邮箱验证码
			...
			verificationCodeBtn.onclick = (event) => {
				...
				// @TODO 通讯
				spa.ajaxRequest(
					App.urls.origin + 'sendVerifyCodeEmail',
					'POST',
	...
	email.init = () => {
		...
		sendEmailCodeBtn.onclick = (event) => {
			...
			// 发送邮箱验证码
			spa.ajaxRequest(
				App.urls.origin + 'sendVerfyCodeEmail',
				'POST',
				JSON.stringify({email: emailEle.value}),
				'application/json;charset=UTF-8',
				(status, data) => {
		...
		*/
		...
		verificationImg.onclick = (event) => {  // 切换验证码图片
		  ...
		  const date = new Date().getTime();
			target.src = App.urls.origin + 'getVerifyCode?' + date;
		};
  ...
  home.init = () => {
    ...
    const rootElement = settings.rootElement;
		...
		const accountBtn = rootElement.querySelector('.header-account-icon');
    ...
    const mentorsContainer = mentors.querySelector('.swiper-container');
		...
		const toggleAccountMenu = (menuBox) => {  // 显示/隐藏账户菜单
			if (menuBox.dataset.active === 'true') {  // 隐藏菜单
			  menuBox.classList.remove('active');
				setTimeout(() => {
				  menuBox.classList.add('hidden');
					menuBox.dataset.active = 'false';
				}, 300);
				return;
			}
			...
			setTimeout(() => {
				...
				menuBox.dataset.active = 'true';
			}, 17);
			return;
		};

		const loadLoginModule = () => {  // 加载登陆模块
			$.uriAnchor.setAnchor({
			  forum: 'welcome', _forum: { login: 'login' }
			});
			return;
		...
		const loadRegisterModule = () => {  // 加载注册模块
		  $.uriAnchor.setAnchor({
			  forum: 'welcome', _forum: { register: 'email' }
			});
			return;
		};

		const updateLoginState = (state) => {  // 更新用户登陆状态
		  globalCache.isLogin = state;
			return;
		};

		const showLogoutTip = () => {  // 用户退出登陆后显示提示信息
			const tipsWrapEle = rootElement.querySelector('.forum .tips');
			const tipsEle = tipsWrapEle.querySelector('.forum .tips-logout');

			// 显示退出登陆后的提示信息
			tipsWrapEle.querySelector('span').textContent = '您已退出登陆';
			tipsWrapEle.classList.remove('hidden');
			tipsEle.classList.add('active');

			// 隐藏退出登陆后的提示信息
			setTimeout(() => {
				tipsEle.classList.remove('active');
			}, 1000);

			if (!tipsEle.classList.contains('active')) {
				tipsWrapEle.classList.add('hidden');
			}
			return;
		};
		...
		accountBtn.onclick = (event) => {
			...
			const menuBoxLoginBtn = menuBox.querySelector('.header-account-menu-login');
			const menuBoxRegisterBtn = menuBox.querySelector('.header-account-menu-register');
			const menuBoxLogoutBtn = menuBox.querySelector('.header-account-menu-logout');

			if (globalCache.isLogin) {  // 用户登陆后隐藏账户选项中的登陆和注册按钮
				menuBoxLoginBtn.classList.add('hidden');  // 隐藏登陆按钮
				menuBoxRegisterBtn.classList.add('hidden');  // 隐藏注册按钮
				menuBoxLogoutBtn.classList.remove('hidden');  // 显示退出登陆按钮
			}
			else {  // 用户未登陆隐藏退出登陆按钮
				menuBoxLogoutBtn.classList.add('hidden');  // 隐藏退出登陆按钮
				menuBoxLoginBtn.classList.remove('hidden');  // 隐藏登陆按钮
				menuBoxRegisterBtn.classList.remove('hidden');  // 隐藏注册按钮
			}

			toggleAccountMenu(menuBox);  // 显示/隐藏菜单

			menuBoxLoginBtn.onclick = (event) => {  // 点击账户菜单中的登陆按钮
				loadLoginModule();  // 加载登陆模块
			};

			menuBoxRegisterBtn.onclick = (event) => {  // 点击账户菜单中的注册按钮
			  loadRegisterModule();  // 加载注册模块
			};

			menuBoxLogoutBtn.onclick = (event) => {  // 用户退出登陆
				spa.ajaxRequest(
				  App.urls.origin + 'logout',
					'GET',
					'',
					'application/x-www-form-urlencoded',
					(status, data) => {
						updateLoginState(false);  // 更新用户登陆状态
						toggleAccountMenu(menuBox);  // 隐藏账户菜单
						showLogoutTip();  // 显示退出登陆后的提示信息
					}
				);
			};
		};
    ...
    tabsItems.forEach((item, index) => {
			item.onclick = () => {
				switch (item.getAttribute('data-tag')) {
					case 'hotQuestion':
						// 请求热门话题数据时，重置为初始请求
						globalCache.hotQuestionDataCount = 0;
					...
					case 'newestQuestion':
						// 请求最新话题数据时，重置为初始请求
						globalCache.newestQuestionDataCount = 0;
    ...
    rootElement.querySelector('.forum .tabs-contents-item')
      .classList.remove('hidden');
		...
		const createNodes = (data) => {
			...
			const rootElement = settings.rootElement;
			const tabsContentsEle = rootElement.querySelector('.tabs-contents');
			const childEle = tabsContentsEle.firstElementChild;
			const vdom = document.createElement('div');
			let dataAnswer = [];
			let count = globalCache.hotQuestionDataCount;

			childEle.classList.remove('hidden');
			// 创建虚拟 DOM
			for (let i = 0, len = dataLen; i < len; i++) {
				const vnode = childEle.cloneNode(true);
			  vdom.appendChild(vnode);

				dataAnswer.push(data[i].infos);
			}

			const vdomNodeList = vdom.childNodes;
			const vdomNodeListLen = vdomNodeList.length;

			// 将后台数据写入虚拟 DOM
			// 1. 渲染用户名
			const usernameEles = vdom.querySelectorAll('.conversation-userinfo-detail-main .name');
			usernameEles.forEach((item, index) => {
			  item.textContent = data[index].username;
			});

			// 2. 渲染浏览量
			const viewEles = vdom.querySelectorAll('.view-text');
			viewEles.forEach((item, index) => {
				item.textContent = data[index].browses;
			});

			// 3. 渲染点赞量
			const thumbEles = vdom.querySelectorAll('.thumb-text');
			thumbEles.forEach((item, index) => {
			  item.textContent = data[index].praises;
				item.dataset.tid = data[index].tid;
			});

			// 4. 渲染话题
			const topicEles = vdom.querySelectorAll('.topic-text');
			topicEles.forEach((item, index) => {
			  item.textContent = data[index].classify;
			});

			// 5. 渲染发布时间
			const dateEles = vdom.querySelectorAll('.date');
			dateEles.forEach((item, index) => {
			  item.textContent = data[index].sendTime;
			});

			// 6. 渲染用户提问内容
			const questionEles = vdom.querySelectorAll('.conversation-question-fullText-para');
			questionEles.forEach((item, index) => {
			  item.textContent = data[index].tMess;
			});

			// 7. 渲染导师回答
			const answerEles = vdom.querySelectorAll('.conversation-answer');
			const answerItemEle = vdom.querySelector('.conversation-answer-mentor-item');
			const vdom2 = document.createElement('div');
			vdom2.classList.add('conversation-answer-mentor-item');

			answerEles.forEach((item, index) => {  // item 回答容器
				const mentorItemWrapEle = item.querySelector('.conversation-answer-mentor');
				const mentorItemEle = item.querySelector('.conversation-answer-mentor-item');
				const dataAnswer = data[index].infos;
				const moreBtn = item.querySelector('.conversation-more');  // 查看更多按钮

				if (dataAnswer.length !== 0) {
				  for (let i = 0, len = dataAnswer.length; i < len; i++) {
						if (item.classList.contains('hidden')) {
						  item.classList.remove('hidden');
						}

						const vmentorItemEle = document.createElement('div');
						vmentorItemEle.className = 'conversation-answer-mentor-item';
						if (i >= 2) { vmentorItemEle.classList.add('hidden'); }

						const vmentorNameEle = document.createElement('span');
						vmentorNameEle.className = 'conversation-answer-mentor-name';
						vmentorNameEle.textContent = dataAnswer[i].username + ': ';

						const vmentorDetailEle = document.createElement('span');
						vmentorDetailEle.className = 'conversation-answer-mentor-detail';
						vmentorDetailEle.textContent = dataAnswer[i].message;

						vmentorItemEle.appendChild(vmentorNameEle);
						vmentorItemEle.appendChild(vmentorDetailEle);

						mentorItemWrapEle.insertBefore(vmentorItemEle, mentorItemWrapEle.lastElementChild);

						// 当导师回答数大于 2 时，隐藏多余的
						if (i > 2) {
							moreBtn.classList.remove('hidden');  // 显示 “展开全部” 按钮
						} else {
			...
			tabsContentsEle.appendChild(vdom);
			childEle.classList.add('hidden');
			beginEvent(data);
		};

		// 调用获取热门提问后台数据
		App.extend.getHotQuestionData(createNodes);

		const beginEvent = (data) => {
			const conversation = new App.extend.Conversation();
			const rootElement = App.settings.rootElement;
			const answerEles = rootElement.querySelectorAll('.conversation-answer-mentor');
			const viewAndThumbEles = rootElement.querySelectorAll('.viewAndThumb');
			const moreBtn = rootElement.querySelector('.tabs-more');

			// 点击展开全部按钮显示所有评论
		  answerEles.forEach((item, index) => {
				const moreBtnEle = item.querySelector('.conversation-more');
				const answerItemEles = item.querySelectorAll('.conversation-answer-mentor-item');

				// 显示隐藏的导师回答
				moreBtnEle.onclick = (event) => {
					const target = event.target;

					answerItemEles.forEach((answerItem, answerIndex) => {
					  if (answerItem.classList.contains('hidden')) {
							answerItem.classList.remove('hidden');
						}
					});
					answerItemEles[0].classList.add('hidden');

					target.parentElement.classList.add('hidden');  // 隐藏按钮
			...
			// 点赞
			// @TODO 点赞成功后，将数据发到后台
			conversation.thumbsUp(viewAndThumbEles, (thumbCount, options) => {
				spa.ajaxRequest(
				  App.urls.origin + 'addPraises',
					'POSt',
					JSON.stringify({tid: parseInt(options.thumbTextEle.dataset.tid, 10)}),
					'application/json;charset=UTF-8',
					(status, data) => {
						const isLogin = globalCache.isLogin;

						// 先在本地检查是否登陆
						if (!isLogin) {
							options.thumbIconEle.classList.remove('active');
							options.thumbIconEle.classList.add('normal');
							thumbCount = options.thumbTextEle.textContent = thumbCount - 1;
						  alert('请先登陆！');
							return;
						}
			...
			if (data.length === 0) {
				moreBtn.querySelector('span').textContent = '没有更多内容啦~';
			  return false;
			}

			// @TODO 页面滚动到底部后请求新的数据并渲染
			moreBtn.onclick = (event) => {
				const target = event.target;
				moreBtn.querySelector('span').textContent = '加载更多';
				App.extend.getHotQuestionData(createNodes);
			};
		};
  ...
  newestQuestion.init = () => {
    ...
    rootElement.querySelector('.forum .tabs-contents-item')
      .classList.remove('hidden');
		...
		// 获取后台最新提问数据
		App.extend.getNewestQuestionData(createNodes);

		// 点击事件处理程序
		const beginEvent = (data) => {
			const moreBtnEle = rootElement.querySelector('.tabs-more');

			if (data.length === 0) {  // 请求数据长度为 0 时
				moreBtnEle.querySelector('span').textContent = '没有更多内容啦~';
			  return;
			}

			moreBtnEle.onclick = () => {  // 点击 "加载更多" 按钮，请求新数据
				moreBtnEle.querySelector('span').textContent = '加载更多';
				App.extend.getNewestQuestionData(createNodes);
			};
		};
  };
		...
		ele.forEach((item, index) => {
			item.onclick = (event) => {
				...
				if (target.classList.contains('thumb-icon') || target.classList.contains('thumb-text')) {
					...
					// ...
					if (typeof callback === 'function') {
						callback(parseInt(thumbCount, 10), {thumbIconEle, thumbTextEle});
					}
				}
			};
		});
  ...
  /**
   * 监听滚动条
	 * @func App.extend.watchScroll
	*/
  App.extend.watchScroll = () => {
	  console.log('监听滚动条');
	};

	// 获取热门提问后台数据
	App.extend.getHotQuestionData = (callback) => {
		let count = globalCache.hotQuestionDataCount ++;
		spa.ajaxRequest(
			App.urls.origin + 'getHotTopic',
			'POST',
			JSON.stringify({uid: count}),
			'application/json;charset=UTF-8',
			(status, data) => {
				if (status === 200 && status !== 404) {
  ...
  // 获取最新提问后台数据
  App.extend.getNewestQuestionData = (callback) => {
	  let tid = globalCache.newestQuestionDataCount;
		spa.ajaxRequest(
		  App.urls.origin + 'getNewTopic',
			'POST',
			JSON.stringify({tid: tid}),
			'application/json;charset=UTF-8',
			(status, data) => {
				if (status === 200 && status !== 404) {
				  if (typeof callback === 'function') { callback(data); }
					...
					const dataLen = parseData.length;
					const lastTid = parseData[dataLen - 1].tid;
					globalCache.newestQuestionDataCount = lastTid;
					return;
				}
