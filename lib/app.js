  ...
  updating.init = () => {
	...
	register.init = () => {
		...
		*/
		const validatorFunc = () => {
		  const validator = new App.extend.Validator();
			...
			validator.add(nicknameEle, [{  // 昵称验证规则
			  strategy: 'isNonEmpty',  // 昵称不为空
				errorMsg: '请输入昵称'
			}, {
				strategy: 'minLength: 3',  // 验证昵称长度
				errorMsg: '昵称长度最少需要 3 个字符'
			}]);
		...
		/**
		 * input 失去焦点后验证
		 * @func onBlurCheck - input 失去焦点后同后台验证
		 * @params {str} url - 后台地址
		 * @params {obj} obj - 发送到后台的数据
		 * @params {ele} ele - 错误信息元素
		 * @params {str} msg - 自定义错误信息
		 * @params {func} callback - 回调接口
		 *
		*/
		const onKeyupCheck = (url, sendData, ele, msg, callback) => {
		  spa.ajaxRequest(
			  url, 'POST', JSON.stringify(sendData),
				'application/json;charset=UTF-8',
				(status, data) => {
					// 通讯成功后执行下面代码
					if (status === 200) {
						...
						if (data === 'false') {
							...
							cache.push(msg);  // 缓存错误信息，点击 注册 按钮时用
						...
						if (data === 'true') {
							...
							cache.pop();  // 移除缓存的错误信息，点击 注册 按钮时用
		...
		/**
		 * 部分 input 失去焦点后调用 onBlurCheck() 方法验证
		 * @func onBlurAction
		*/
		const onKeyupAction = (() => {
			const nicknameEle = rootElement.querySelector('.register-container-nickname input');
			...
			const emailCode = rootElement.querySelector('.register-container-verificationCode input');
			const phoneEle = rootElement.querySelector('.register-container-phone input');
			const verificationTextTip = rootElement.querySelector('.register-tips-verification-text');

			nicknameEle.onkeyup = () => {  // 昵称失去焦点验证(input)
				onKeyupCheck(
					'http://wd.jvziqinggan.com/Forum/checkUsername',
					{username: nicknameEle.value},
					verificationTextTip,
					'用户昵称已存在！'
				);
			};

			emailEle.onkeyup = () => {  // 用户邮箱失去焦点验证(input)
				const validator = new App.extend.Validator();
				...
				// 邮箱地址验证不通过时，获取错误信息，并阻止与后台通信验证邮箱号是否
				// 存在
				const errorMsg = validator.start();
				if (errorMsg) {  // 存在错误信息时，发送邮件验证码按钮不可点击
					sendEmailCodeBtn.classList.add('unclickable');
					return;
				}
				sendEmailCodeBtn.classList.remove('unclickable');

				// 当邮箱不为空且邮箱格式无误后，在与后台通信，验证该邮箱号是否可用
				onKeyupCheck(
					'http://wd.jvziqinggan.com/Forum/checkEmail',
					{email: emailEle.value}, verificationTextTip,
					'该邮箱已存在！'
				);
			};

			emailCode.onkeyup = () => {  // 邮箱验证码失去焦点验证
			  const validator = new App.extend.Validator();

				validator.add(emailCode, [{  // 邮箱验证码规则
				  strategy: 'isNonEmpty',  // 验证邮箱验证码是否为空
					errorMsg: '请输入验证码！'  // 验证不通过时的错误信息
				}, {
					strategy: 'isLength: 6',  // 验证邮箱验证码是否为 6 位
					errorMsg: '请输入 6 位验证码！'  // 验证不通过时的错误信息
				}]);

				const errorMsg = validator.start();  // 前端验证不通过，获取错误信息
				if (errorMsg) {
				  console.log('请输入 6 位验证码');
					return;
				}

				onKeyupCheck(
				  'http://192.168.2.147:8080/checkVerifyCodeEmail',
					{verifyCode: emailCode.value}, verificationTextTip,
					'验证码错误'
				);
			};
		
			phoneEle.onkeyup = () => {  // 手机号失去焦点验证(input)
				const validator = new App.extend.Validator();

				validator.add(phoneEle, [{  // 手机号验证规则
				  strategy: 'isNonEmpty',  // 验证手机号是否为空
					errorMsg: '请输入手机号'  // 前端验证不通过时的错误信息
				}, {
				  strategy: 'isMobile', // 验证手机号格式是否正确
					errorMsg: '请输入正确的手机号'  // 前端验证不通过时的错误信息
				}]);

				const errorMsg = validator.start();  // 前端验证不通过时获取错误信息

				if (errorMsg) { return; }

				// 前端验证通过后，与后台通信，验证手机号是否可用
				onKeyupCheck(
					'http://wd.jvziqinggan.com/Forum/checkTel',
					{tel: parseInt(phoneEle.value, 10)}, verificationTextTip,
					'该手机号已存在！'
				);
			};
		})();
